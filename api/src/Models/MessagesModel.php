...
{
    public function allMessageById(int $id): array
    {
        $stmt = $this->query("
...
ORDER BY rm.date DESC, rm.hour DESC;
 ");
    ...
    public function allMessageByIdById(int $id, int $id2): array
    {
...
")
            ->fetchAll(['id' => $id, 'id2' => $id2]);

    }

    public function addMessage(int $receiverId, int $senderId, string $message, string $date, string $hour): void
    {
        $stmt = $this->query("
...
");
    ...
    public function updateMessage(int $id, int $id2): bool
    {
        $stmt = $this->query("
            UPDATE messages 
            SET is_view = 1
